...
# Environment-driven table names for flexibility
...
supabase_conn: Any = None  # Supabase client instance populated in init_supabase_client

def init_supabase_client():
    global supabase_conn
    try:
        if supabase_conn is None:
            # Environment variables are assumed to be set in production; suppress type checker complaint.
...
def _ensure_client():
    """Initialise Supabase client if necessary."""
    global supabase_conn
    if supabase_conn is None:
        init_supabase_client()
...
def get_user_by_telegram_id(telegram_id: int) -> Optional[dict]:
    """Return first user dict by telegram_id or None."""
    _ensure_client()
    result = (
        supabase_conn
        .table(USERS_TABLE)
...
def create_user(telegram_id: int) -> dict:
    """Create user with autogenerated email; return inserted row."""
    _ensure_client()
...
def save_one_time_token(token_hash: str, telegram_id: int, ttl_seconds: int) -> None:
    """Insert a one-time token hash."""
    _ensure_client()
    # Clean up: remove any previous tokens for this telegram_id or already expired tokens
    now = _now_utc()
...
def _purge_old_refresh_tokens(user_id: str):
    """Remove existing tokens for user and global expired tokens to keep table tidy."""
    _ensure_client()

    # 1. Delete all tokens for this user (they will be replaced by a fresh one)
    supabase_conn.table(REFRESH_TOKENS_TABLE).delete().eq("user_id", user_id).execute()

    # 2. Additionally, delete any tokens globally that are expired
